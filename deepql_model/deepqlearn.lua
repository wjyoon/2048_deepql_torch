...
function Brain.init(num_states, num_actions)
   ...
   Brain.start_learn_threshold = 3000
   ...
   Brain.net:add(nn.Linear(Brain.hidden_nodes, 500))
   Brain.net:add(nn.Threshold(0,0))

   Brain.net:add(nn.Linear(500, Brain.net_outputs))
   
   Brain.criterion = nn.MSECriterion()
   
   
   -- other learning parameters
   Brain.learning_rate = 0.00005;
...
function Brain.save_tbl(tbl, name)
  dofile("deepql_model/table_save.lua")
  assert(table.save(tbl, name .. "_brain.tbl") ==1 )
end

function Brain.save_Brain()
  dofile("deepql_model/table_save.lua")
  assert (table.save(Brain,"Brain.tbl") == 1 )
end

function Brain.load_Brain()
  dofile("deepql_model/table_save.lua")
  Brain = table.load("Brain.tbl")
end
...
function Brain.save(name, network, experiences)
  torch.save(name .. ".net", network)
end

function Brain.load(name)
  Brain.net = torch.load(name)
end 
        ...
        for k = 1, Brain.batch_size do
          ...
          -- copy state from experience
		  	...
		  	targets[k] = all_outputs:clone();
                        print ("e.reward0  ", e.reward0)
                        print("best_action.value ", best_action.value)
